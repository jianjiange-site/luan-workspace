...
所有共享基建资源都按拼音前缀隔离，本仓库统一用 **`luan`**：

| 资源                     | 取值                                                |
|------------------------|---------------------------------------------------|
| PG 库                   | `dating-dev-luan`                                 |
| Redis key 前缀           | `luan:<service>:<domain>:<id>`                    |
| Nacos namespace        | `dev-luan`                                        |
| MinIO bucket           | `dating-luan`                                     |
| RocketMQ topic / group | `dev_luan_*`                                      |
| Proto 包坐标              | `com.dating.luan.proto:*` / `dating-proto-luan-*` |
| OpenIM userID          | `<service>_luan_*`                                |

文档里凡是写 `<name>` / `<yourname>` / `<yourpinyin>` 的地方，都替换成 `luan`。
...
- [`docs/match-service-prd-tech.md`](docs/match-service-prd-tech.md) — match-service 业务设计草案
...
7. **`.proto` 走 Nexus 包**：proto 文件放 `proto/`，发布坐标必须带 `luan` 前缀。
...
- `dev` 集成主线，个人分支 PR 合入
- 个人分支命名：`luan/<topic>`，例 `luan/post-feed-rank`
